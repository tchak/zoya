...
TypeError (structured enum)
  → zoya-run: EvalError::RuntimeError(String)  ← still flattens
...
### 1. `zoya-run` — Eliminate remaining `Result<_, String>` (Small)
...
### 4. `zoya-loader` — Minor cleanup (Small)
...
1. **`zoya-run`** — Wrap errors properly with `EvalError`
2. **`zoya-std`** — Add `StdError`, eliminate `Result<_, String>`
3. **`zoya` CLI** — Adopt `anyhow` or `CliError`, propagate structured errors
4. **`zoya-loader`** — Preserve upstream error types instead of extracting `.message`
