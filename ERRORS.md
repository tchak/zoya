...
| `zoya-run` | `EvalError` | `Panic`, `RuntimeError` | Main runtime error type |
...
zoya-parser::ParseError ───────┘         |                     |
                                         |                     |
zoya-package::ConfigError ─ .to_string() ┘                     |
                                                               |
                                                               ├──> CLI: anyhow::Result
zoya-ir::TypeError ────────────────────────────────────────────┤
   (from zoya-check)                                           |
...
- **`ConfigError` into `LoaderError`**: Still flattened to `String` via `.to_string()`.
- **`LoaderError`/`TypeError` into `EvalError`**: Flattened to `EvalError::RuntimeError(e.to_string())` — loses structure.
...
- **`LoaderError::ConfigError(String)`** — should embed `ConfigError` directly instead of flattening
- **`EvalError`** — should wrap `LoaderError` and `TypeError` as `#[source]` instead of `.to_string()`
