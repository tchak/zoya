...
Every crate uses [`thiserror`](https://github.com/dtolnay/thiserror) for structured error enums. The CLI boundary uses [`anyhow`](https://github.com/dtolnay/anyhow) for ergonomic error propagation. Lex and parse errors are rendered with source annotations using [`miette`](https://github.com/zkat/miette) (CLI crate only).
...
| `zoya-loader` | `LoaderError<P>` | `ModuleNotFound`, `DuplicateMod`, `SourceError`, `LexError`, `ParseError`, `InvalidModName`, `ReservedModName`, `NoPackageToml`, `MainNotFound`, `ConfigError`, `MissingRoot`, `InvalidAttribute` | Generic over path type; embeds `LexError`/`ParseError` as `#[source]` with `source_text` for diagnostics |
...
Lex and parse errors are rendered with source-annotated diagnostics via [`miette`](https://github.com/zkat/miette). The `diagnostic` module in the CLI crate (`crates/zoya/src/diagnostic.rs`) downcasts `anyhow::Error` to `LoaderError` or `EvalError`, extracts the embedded `source_text` and span information, and renders colorized, source-pointing error messages to stderr.

For errors that are not lex/parse errors (type errors, runtime errors, etc.), the `fatal()` function prints `"error: <message>"` in red/bold to stderr using the `console` crate.

The REPL and `fmt` command also use miette rendering for inline lex/parse errors where source text is directly available.
...
- **Rich diagnostics for type errors** — `TypeError` spans could be rendered with miette similarly to lex/parse errors
