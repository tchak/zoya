...
use axum::routing;
use zoya_build::{BuildOutput, HttpMethod};
...
pub fn router(output: &BuildOutput) -> Router {
    ...
    for (path, _, _) in &output.routes {
        let func = output
            .definitions
            .get_function(path)
            .expect("route function must exist in definitions");
        route_infos.push(RouteInfo {
            path: path.clone(),
            has_request_param: !func.params.is_empty(),
        });
    }

    // Collect route metadata before moving output into Arc
    let routes_meta: Vec<_> = output
        .routes
        .iter()
        .map(|(_, method, pathname)| (*method, convert_pathname(pathname.as_str())))
        .collect();
    ...
    for (i, (method, axum_path)) in routes_meta.iter().enumerate() {
...
async fn handle_request(
    ...
    let result = zoya_run::run_async(&state.output, info.path.clone(), args).await;
